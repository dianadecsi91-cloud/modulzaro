...
constructor(router: Router){
  ...
  localStorage.setItem("dev_WS", JSON.stringify(this.products))

}
Add():void{
   let jsonArray = JSON.parse(localStorage.getItem("ang_WS") ?? "[]")
    let products: Product[] = Object.values(jsonArray).map(x => Object.assign(new Product(), x))

  
    this.products.push(this.product)

    localStorage.setItem("ang_WS", JSON.stringify(this.products))
